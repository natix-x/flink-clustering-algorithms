...
public class DistributedFastPAM implements Clusterer {
    ...
    private static final class Combine
        ...
        public void onEpochWatermarkIncremented(int epoch, Context context, Collector<Update> out) {
            ...
            System.out.printf(
                "[DistributedFastPAM] epoch=%d phase=%s numSelected=%d/%d swapRounds=%d/%d partials=%d prevRound=%.0fms%n",
            ...
            if (s.numSelected < k) {
                ...
                System.out.println("[DistributedFastPAM]   BUILD picked medoid #" + s.numSelected
                ...
                System.out.println("[DistributedFastPAM]   SWAP round " + next.swapRounds + "/" + maxIter
                    + (applied ? "  swap medoid[" + bestI + "] <- point[" + bestH + "]" : "  no improving swap")
                    + "  bestDelta=" + bestDelta);
                if (stop) {
                    System.out.println("[DistributedFastPAM] stop at swapRound=" + next.swapRounds
